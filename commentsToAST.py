...
class P(Parser):
    def program(p):
        ...
        while not p > KRAJ:
            ...
            rt.mem[funkcija.ime] = funkcija # dodaj funkciju u memoriju 
        return p.funkcije
    ...
    #     elif broj := p.broj():
    #         print("broj. p {}")
    #         varijable.append(broj)
    #     elif varijabla := p >= T.CLOSED: ''' im here {varijabla} '''
...
class Povratak(NelokalnaKontrolaToka): """Signal koji salje naredba vrati"""
...
class Funkcija(AST):
    ...
    def izvrši(self, mem=None, unutar=None):
        self.tijelo.izvrši(mem=mem, unutar=unutar)
    def pozovi(self, argumenti):
        lokalni = Memorija(zip(self.parametri, argumenti))
        try: 
            self.izvrši(mem=lokalni, unutar=self)
        except Exception: 
...
class Poziv(AST):
    funkcija: 'Funkcija'
    argumenti: '?'
    # def izvrši(poziv,mem,lokalni): # or myb izvrši 
    #     pozvana = poziv.funkcija
    #     if pozvana is nenavedeno: pozvana = lokalni # rekurzivni poziv 
    #     #mem, lokalni
    ...
    def izvrši(self, mem=None, unutar=None):
        self.funkcija.izvrši(mem=mem, unutar=unutar)
...
class Vrati(AST):
    ...
    vrijednost : 'vri'
    # def izvrši(self, mem=None, unutar=None):
        # return rt.mem[self.ime]
    def izvrši(p,mem, lokalni):
...
class Start(AST):
    ...
    def izvrši(program,mem,unutar):
        # ret_val = None
        for naredba in program.naredbe:
            # ret_val = naredba.izvrši(lokalni)
            ret_val = naredba.izvrši(mem,unutar)
            # if ispunjen(program,mem, unutar): naredba.izvrši(mem,unutar)
            # else: naredba.inače.izvrši(mem,unutar)
            # return naredba.izvrši(mem,unutar) #.vrijednost

            if isinstance(ret_val, Vrati): 
                return ret_val.vrijednost
...
main(){
    ...
    PRINT(x)
    CALL g(y)
    PRINT() // dopusteno je printat prazan string/broj
    for(i = 1; 5; i+=1){
        PRINT(i)}
    CALL f(x)
